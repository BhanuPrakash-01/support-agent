...
def save(fl):  open(PATH, "w").write(json.dumps(fl, indent=2) + "\n")

def validate(fl):
    rules = fl.get("rules", {})
    for f in fl["features"]:
        for k in REQUIRED:
            if k not in f: fail(f"feature {f.get('id', '?')} missing field: {k}")
        if f["state"] not in STATES: fail(f"feature {f['id']} invalid state: {f['state']}")
...
def find(fl, fid):
    ...
    if f is None: fail(f"unknown id: {fid}")
    return f

def main():
    fl = load(); validate(fl)            # malformed list is always a hard error
    rules = fl.get("rules", {})
    ...
    if cmd == "status":
        ...
        if not fid: fail("activate requires an id")
        wip = rules.get("wip_limit", 1)
        if len(active) >= wip: fail(f"WIP={wip}; {active[0]['id']} already active.")
        if vcr < rules.get("vcr_target", 0):
            fail(f"VCR={vcr:.2f} below target; resolve blocked features first.")
        f = find(fl, fid)
        if f["state"] != "not_started": fail(f"{fid} is in state '{f['state']}'")
        f["state"] = "in_progress"; save(fl); print(f"activated {fid}")

    elif cmd == "pass":
        if not fid: fail("pass requires an id")
        f = find(fl, fid)
        if f["state"] != "in_progress": fail(f"{fid} is not in_progress")
        ...
        save(fl); print(f"passed {fid}; evidence recorded")

    elif cmd == "block":
        if not fid: fail("block requires an id")
        f = find(fl, fid); f["state"] = "blocked"
        reason = " ".join(sys.argv[3:])
        if reason: f["notes"] = reason
        save(fl); print(f"blocked {fid}")
